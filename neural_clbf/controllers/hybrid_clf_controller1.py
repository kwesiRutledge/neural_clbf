...
class HybridCLFController1(Controller):
    """
    A CLF-based controller for hybrid control affine systems like the
    HybridControlAffineSystem described in this repository. This uses
    the quadratic Lyapunov function found for the linearized system.
    ...
    def __init__(
        self,
        dynamics_model: HybridControlAffineSystem,
        ...
        """
        super(HybridCLFController1, self).__init__(
    ...
    def V_lie_derivatives(
        ...
        for i in range(n_scenarios):
            ...
            f_all = self.dynamics_model._f_all(x, s)
            g_all = self.dynamics_model._g_all(x, s)

            for mode_index in range(self.dynamics_model.n_modes):
                f_m = f_all[:,:,mode_index]
                g_m = g_all[:,:,:,mode_index]
