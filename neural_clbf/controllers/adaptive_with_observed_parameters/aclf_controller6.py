...
class aCLFController6(Controller):
    """
    ...
    Desperation controller. Let's just barely modify the control logic.
    """
    ...
    def __init__(
        ...
        """
        super(aCLFController6, self).__init__(
        ...
        # Define CVXPyLayer
        self.differentiable_qp_solver = self.define_aclf_optim_cvxpylayer()

    def V_with_jacobian(self, x: torch.Tensor, theta_hat: torch.Tensor, scen: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        args:
            ...
            scen: bs x self.dynamics_model.n_scenario the observed parameters defining the scenario
        returns:
        ...
        x0 = self.dynamics_model.goal_point(theta_hat, scen).type_as(x_theta)
        theta_hat0 = torch.tensor(
            self.dynamics_model.sample_polytope_center(pc.qhull(self.dynamics_model.V_Theta.numpy()))
    ...
    def V(self, x: torch.Tensor, theta_hat: torch.Tensor, scen: torch.Tensor) -> torch.Tensor:
        """Compute the value of the CLF"""

        V, _, _ = self.V_with_jacobian(x, theta_hat, scen)
        return V

    def V_lie_derivatives(
        self, x: torch.Tensor, theta_hat: torch.Tensor, scen: torch.Tensor,
        ...
        returns:
            Lf_V: bs x 1 tensor of Lie derivatives of V
                  along f
            LF_V: bs x self.dynamics_nodel.n_params tensor
                    of Lie derivatives of V along F
            LFGammadV_V: bs x 1 tensor
            Lg_V: bs x self.dynamics_model.n_controls tensor
                  of Lie derivatives of V along g
            list_LGi_V: list of bs x self.n_controls tensors each one representing
                        the Lie derivatives of V along G_i (one i for each dimension of the unknown parameter theta)
            LGammadVG_V: bs x self.dynamics_model.n_controls

        usage:
            Lf_V, LF_V, LFGammadV_V, Lg_V, list_LGi_V, LGammadVG_V = self.V_lie_derivatives(x, theta_hat, scen)
        """
        # Constants
        ...
        V, gradV_x, gradV_theta = self.V_with_jacobian(x, theta_hat, scen)

        # We need to compute Lie derivatives for each scenario
        batch_size = x.shape[0]
        Lf_V = torch.zeros((batch_size, 1), device=x.device)
        LF_V = torch.zeros((batch_size, self.dynamics_model.n_params), device=x.device)
        LFGammadV_V = torch.zeros(batch_size, 1, device=x.device)
        Lg_V = torch.zeros(batch_size, self.dynamics_model.n_controls, device=x.device)
        LGammadVG_V = torch.zeros(batch_size, self.dynamics_model.n_controls, device=x.device)
        ...
        for param_index in range(self.dynamics_model.n_params):
            LGi_V = torch.zeros(batch_size, self.dynamics_model.n_controls)
        ...
        f = self.dynamics_model._f(x, scen)
        F = self.dynamics_model._F(x, scen)
        g = self.dynamics_model._g(x, scen)
        G = self.dynamics_model._G(x, scen)

        # Multiply these with the Jacobian to get the Lie derivatives
        Lf_V[:, :] = torch.bmm(gradV_x, f).squeeze(1)
        LF_V[:, :] = torch.bmm(gradV_x, F).squeeze(1)
        Lg_V[:, :] = torch.bmm(gradV_x, g).squeeze(1)

        LF_V_as_rows = LF_V[:, :].unsqueeze(1)
        LFGamma = torch.bmm(LF_V_as_rows, Gamma_copied)
        LFGammadV_V[:, :] = torch.bmm(LFGamma, gradV_theta.mT).squeeze(1)

        # list_LGi_V
        for param_index in range(self.dynamics_model.n_params):
            G_p = G[:, :, :, param_index].reshape((batch_size, self.dynamics_model.n_dims, self.dynamics_model.n_controls))
            list_LGi_V[param_index][:, :] = torch.bmm(gradV_x, G_p).squeeze(1)
        ...
        for mode_index in range(self.dynamics_model.n_params):
            ...
            LGammadVG_V_current[:, :, :] = LGammadVG_V[:, :].unsqueeze(1)
            ...
            LGammadVG_V[:, :] = (LGammadVG_V_current + torch.matmul(gradV_x, GammadVG_i)).squeeze(1)
    ...
    def Vdot_for_scenario(self, x: torch.Tensor, theta_hat: torch.Tensor, scen: torch.Tensor, u: torch.Tensor) -> torch.Tensor:
        """
        Vdot_for_scenario
        Description
        ...
        Lf_Va, LF_Va, LFGammadVa_Va, Lg_V, list_LGi_V, LGammadVG_V = self.V_lie_derivatives(x, theta_hat, scen)
        # print(
        #     "list_LGi_V = \n", list_LGi_V, "\n",
        # )

        # Use the dynamics to compute the derivative of V at each corner of V_Theta
        sum_LG_V = torch.zeros((bs, n_controls), device=x.device)
        for theta_dim in range(n_params):
            sum_LG_V = sum_LG_V + \
                       torch.bmm(
                           theta_hat[:, theta_dim].reshape((bs, 1, 1)),
                           list_LGi_V[theta_dim].unsqueeze(1),
                       ).to(x.device).squeeze()

        # Should we use the true theta in this computation? I don't think so.
        Vdot = Lf_Va[:, :].unsqueeze(1) + \
            torch.bmm(
                LF_Va[:, :].unsqueeze(1),
                theta_hat.reshape((bs, self.dynamics_model.n_params, 1)),
            ) + \
            LFGammadVa_Va[:, :].unsqueeze(1) + \
            torch.bmm(
                Lg_V[:, :].unsqueeze(1) + sum_LG_V.unsqueeze(1) + LGammadVG_V.unsqueeze(1),
                u.reshape(-1, self.dynamics_model.n_controls, 1),
           )

        return Vdot

    def u_reference(self, x: torch.Tensor, theta_hat: torch.Tensor, scen: torch.Tensor) -> torch.Tensor:
        """Determine the reference control input."""
        # Here we use the nominal controller as the reference, but subclasses can
        # override this
        return self.dynamics_model.u_nominal(x, theta_hat, scen)
    ...
    def _solve_CLF_QP_gurobi(
        ...
        Theta_vertices = center_and_radius_to_vertices(
            theta_hat.clone().to(theta_hat.device),
            eps0 * torch.ones(theta_hat.shape, device=theta_hat.device),
        )
        n_Theta_vertices = len(Theta_vertices)
        ...
        for v_Theta_index in range(n_Theta_vertices):
            ...
            theta_corner = Theta_vertices[v_Theta_index]

            # Compute Lie Derivatives here
            Lf_V, LF_V, LFGammadV_V, Lg_V, list_LGi_V, LGammadVG_V = self.V_lie_derivatives(x, theta_corner, scen)
        ...
        for batch_idx in range(bs):
            ...
            for v_idx in range(n_Theta_vertices):
                ...
                Lf_V_np = Lf_V_torch[batch_idx, :].detach().cpu().numpy()

                LF_V_torch = Thetas_LF_V[v_idx]
                LF_V_np = LF_V_torch[batch_idx, :].detach().cpu().numpy()

                LFGammadV_V_torch = Thetas_LFGammadV_V[v_idx]
                LFGammadV_V_np = LFGammadV_V_torch[batch_idx, :].detach().cpu().numpy()

                Lg_V_torch = Thetas_Lg_V[v_idx]
                Lg_V_np = Lg_V_torch[batch_idx, :].detach().cpu().numpy()

                list_LGi_V_np = []
                list_LGi_V_torch = Thetas_list_LGi_V[v_idx]
                for theta_dim in range(n_params):
                    list_LGi_V_np.append(
                        list_LGi_V_torch[theta_dim][batch_idx, :].detach().cpu().numpy()
                    )

                LGammadVG_V_torch = Thetas_LGammadVG_V[v_idx]
                LGammadVG_V_np = LGammadVG_V_torch[batch_idx, :].detach().cpu().numpy()
                ...
                model.addConstr(clf_constraint <= 0.0, name=f"Batch {batch_idx}, Corner {v_idx} Decrease")
            ...
            if model.status != GRB.OPTIMAL:
                ...
                if allow_relaxation:
                    r_result[batch_idx] = torch.tensor(float("nan"))
                continue

            # Extract the results
            for i in range(n_controls):
                u_result[batch_idx] = torch.tensor(u.x)
            if allow_relaxation:
                    r_result[batch_idx] = torch.tensor(r.x)
    ...
    def _solve_CLF_QP_cvxpylayers(
        ...
        for v_Theta_idx in range(n_V_Theta):
            ...
            params.append(LFGammadV_V_corner[:, :])
    ...
    def solve_CLF_QP(
        ...
        if u_ref is not None:
            ...
            u_ref = self.u_reference(x, theta_hat, scen)
        ...
        if requires_grad:
            if (self.diff_qp_layer_to_use == "cvxpylayer") or (self.diff_qp_layer_to_use == "cvxpylayers"):
                ...
                return self._solve_CLF_QP_qpth(
                    x, theta_hat, u_ref, V, relaxation_penalty=relaxation_penalty, Q_u=Q,
                )
            else:
                raise ValueError(
    ...
    def tau(self, x: torch.Tensor, theta_hat: torch.Tensor, u: torch.Tensor, scen: torch.Tensor) -> torch.Tensor:
        ...
        _, dVdx, dVdth = self.V_with_jacobian(x, theta_hat, scen)
        ...
        LF_V_scenario[:, :, :] = LF_V[:, :].unsqueeze(dim=1)
    ...
    def closed_loop_estimator_dynamics(self, x: torch.Tensor, theta_hat: torch.Tensor, u: torch.Tensor, scen: torch.Tensor):
        """
        ...
        tau = self.tau(x, theta_hat, u, scen)
    ...
    def V_oracle(
        ...
        args:
            ...
            scen: bs x self.dynamics_model.n_scenario tensor of scenario values
        """
        # Constants
        bs = x.shape[0]
        Gamma = self.Gamma.to(x.device)
    ...
    def define_aclf_optim_cvxpylayer(self):
        """
        define_aclf_optim_cvxpylayer
        Description:
            This function defines a CvxPyLayer object which solves the QP for the aCLF optimization.
        """
        ...
        LGammadVaG_param_cluster, LFGammadV_Va_param_cluster, LG_Va_cluster = [], [], []
        ...
        Lf_Va_params = Lf_Va_param_cluster
        Lg_Va_params = Lg_Va_param_cluster
        LF_Va_params = LF_Va_param_cluster
        LGammadVaG_params = LGammadVaG_param_cluster
        LFGammadV_Va_params = LFGammadV_Va_param_cluster
        LG_Va_params = LG_Va_cluster
        ...
        for v_Theta_index in range(n_Theta_v):
            ...
            sum_LG_i_Va = v_Theta[0] * LG_Va_params[v_Theta_index][0]
            for theta_index in range(1, len(v_Theta)):
                sum_LG_i_Va = sum_LG_i_Va + v_Theta[theta_index] * LG_Va_params[v_Theta_index][theta_index]

            # CLF decrease constraint (with relaxation)
            constraints.append(
                Lf_Va_params[v_Theta_index]
                + LF_Va_params[v_Theta_index] @ v_Theta + LFGammadV_Va_params[v_Theta_index]
                + (Lg_Va_params[v_Theta_index] + sum_LG_i_Va + LGammadVaG_params[v_Theta_index]) @ u
        ...
        parameters = parameters + Lf_Va_params + Lg_Va_params + LF_Va_params
