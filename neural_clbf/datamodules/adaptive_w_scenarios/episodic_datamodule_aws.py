"""
episodic_datamodule_aws.py
Description:
    Data module for episodic training with unknown parameters and scenario vectors.
"""
...
class EpisodicDataModuleAdaptiveWScenarios(pl.LightningDataModule):
    """
    ...
    def __init__(
        self,
        model: ControlAffineParameterAffineSystem2,
        ...
        V_Theta = model.V_Theta.cpu().numpy()
        ...
        # Save the min, max, central point, and range tensors for the scenario vectors
        self.scenarios_max, self.scenarios_min = model.scenario_limits
        self.scenario_center = (self.scenarios_max + self.scenarios_min)/2.0
        self.scenario_range = self.scenarios_max - self.scenarios_min
    ...
    def sample_trajectories(
        ...
        s_init = torch.zeros((self.trajectories_per_episode, self.model.n_scenario), device=self.device)
    ...
    def sample_fixed(self) -> torch.Tensor:
        ...
        for region_name, quota in self.quotas.items():
            ...
            if region_name == "goal":
                _, x_sample_group, theta_sample_group, scen_sample_group = self.model.sample_goal(num_samples)
            elif region_name == "safe":
                _, x_sample_group, theta_sample_group, scen_sample_group = self.model.sample_safe(num_samples)
            elif region_name == "unsafe":
                _, x_sample_group, theta_sample_group, scen_sample_group = self.model.sample_unsafe(num_samples)
            elif region_name == "boundary":
                _, x_sample_group, theta_sample_group, scen_sample_group = self.model.sample_boundary(num_samples)

            # Append new samples to the respective lists
            x_samples.append(x_sample_group)
            theta_samples.append(theta_sample_group)
            scen_samples.append(scen_sample_group)
        ...
        # Sample scenario values
        scen_samples.append(self.model.sample_scenario_space(free_samples))
        ...
        return torch.vstack(x_samples), torch.vstack(theta_samples), torch.vstack(theta_hat_samples), torch.vstack(scen_samples)

    def prepare_data(self):
        """Create the dataset"""
        # Get some data points from simulations
        x_sim, theta_sim, theta_hat_sim, scen_sim = self.sample_trajectories(self.model.nominal_simulator)
        ...
        scen_vec = torch.cat((scen_sim, scen_sample), dim=0)
        ...
        self.scen_training = scen_vec[training_indices]
        self.scen_validation = scen_vec[validation_indices]
