...
from neural_clbf.experiments.adaptive.safety_case_study import (
    CaseStudySafetyExperiment, create_initial_states_parameters_and_estimates,
    tabulate_number_of_reaches, counts_to_latex_table, save_timing_data_table,
    CaseStudySafetyExperimentTrajOpt2, CaseStudySafetyExperimentMPC,
)
...
def create_case_study_experiments(
    ...
    safety_case_study_experiment = CaseStudySafetyExperiment(
        ...
        n_sims_per_start=1,
        t_sim=t_sim,
        plot_x_indices=[ScalarCAPA2Demo.X_DEMO],
        plot_x_labels=["$s_x$"],
    )
...
def main(args):
    ...
    # Select Controller
    controller_to_test = controller_pt
    controller_to_test.Q_u = np.diag([1.0])
    ...
    controller_to_test.experiment_suite = ExperimentSuite([
        safety_case_study_experiment, safety_trajopt_exp, safety_mpc_exp,
    ])
    ...
    def scalar_update(t, x, u, params):
        """
        dxdt = scalar_update(t,x,u, params)
        Description:
        ...
        return (1+theta)*s_x + (1+theta)*s_x
    # Create obstacle
    obs_center = np.array([
        dynamics_model.nominal_scenario["wall_position"],
    ])
    constraints = []
    constraints.append(
        (
            optimize.NonlinearConstraint,
            lambda x, u: np.linalg.norm(x[0] - obs_center[0]),
            0.5,
            float('Inf'),
        )
    )
    ...
    mpc_results_df, mpc_trajopt_synthesis_times, _, _, _, _  = safety_mpc_exp.run(
        controller_to_test.dynamics_model, controller_to_test.controller_period,
        scalar_update,
        Tf=t_sim,
        P=np.diag([1.0e3]),
    ...
    save_timing_data_table(
        "../datafiles/scalar_demo_capa2/safety_case_study_timing_results.txt",
...
def extract_hyperparams_from_args2(args):
    """
    ...
    saved_hyperparams["num_init_epochs"] = 15
    ...
    V_contour_experiment = AdaptiveCLFContourExperiment(
        "V_Contour",
        x_domain=[
            (dynamics_model.state_limits[1][AdaptivePusherSliderStickingForceInput.S_X],
             dynamics_model.state_limits[0][AdaptivePusherSliderStickingForceInput.S_X]),
        ],
...
def inflate_context_using_hyperparameters2(hyperparams):
    """
    dynamics, scenarios, data_module, experiment_suite, x0 = inflate_context_using_hyperparameters(saved_hyperparams)
    Description:
        Inflates some useful constants from the hyperparameters file that was loaded.
    ...
    obs_rad = nominal_scenario["obstacle_radius"]
...
def create_case_study_experiments2(
        x0: torch.tensor,
        theta_hat0: torch.tensor,
        theta0: torch.tensor,
        t_sim: float,
...
def main2(args):
    ...
    # - MPC Controller Safety Testing
    mpc_results_df, _, _, _, _, _ = safety_mpc_exp.run(
        controller_to_test.dynamics_model, controller_to_test.controller_period,
        lsm_update,
        Tf=t_sim,
        P=np.diag([1.0e3, 1.0e3, 1.0e3]),
