"""
eval_safety_and_timing_adaptive_pusher_slider.py
Description:
    In this file, I will evaluate the safety and timing of the adaptive
    pusher slider system using the 4 different controllers we had in mind
    for this system.
"""
...
from neural_clbf.systems.adaptive import (
    AdaptivePusherSliderStickingForceInput, ControlAffineParameterAffineSystem
)
...
from neural_clbf.experiments.adaptive_w_observed_parameters.safety_case_study import (
...
def extract_hyperparams_from_args(args):
    ...
    scalar_capa2_log_file_dir = "../../training/adaptive_w_observed_parameters/logs/pusher_slider_sticking_force_input/"
    ...
    P_scenario = pc.box2poly(
        np.array([saved_hyperparams["scenario_lb"], saved_hyperparams["scenario_ub"]]).T,
    )
    ...
    controller_from_state_dict = NeuralaCLBFControllerV5(
        dynamics_model,
        data_module,
...
def inflate_context_using_hyperparameters(hyperparams):
    ...
    obs = torch.tensor([
        nominal_scenario["obstacle_0_center_x"],
        nominal_scenario["obstacle_0_center_y"],
    ])
    obs_rad = 0.1
    # start_x = torch.tensor(
    #     [
    #         [-0.3,                obs[1]-1.5*obs_rad,     0.1],
    #         [-0.3,                obs[1]-4.5*obs_rad,     0.0],
    #         [-0.3,                obs[1]-4.5 * obs_rad,   0.0],
    #         [obs[0]-0.5*obs_rad,    obs[1]-4.5*obs_rad,     0.0],
    #         [obs[0]-0.5*obs_rad,    obs[1]-4.5*obs_rad,     0.1],
    #         [obs[0]-0.5*obs_rad,    obs[1]-4.5 * obs_rad,   0.0],
    #         [obs[0]-0.5*obs_rad,    obs[1]-4.5*obs_rad,     0.0],
    #         [obs[0]+0.5*obs_rad,    obs[1]-4.5*obs_rad,     0.0],
    #         [obs[0]+0.5 * obs_rad,  obs[1]-4.5*obs_rad,     0.0],
    ...
    # Define the dynamics model
    P_scenario = pc.box2poly(
        np.array([hyperparams["scenario_lb"], hyperparams["scenario_ub"]]).T,
    )
    dynamics_model = PusherSlider(
        nominal_scenario,
        Theta,
        P_scenario=P_scenario,
        dt=hyperparams["simulation_dt"],
        controller_dt=hyperparams["controller_period"],
    )
...
def create_case_study_experiments(
    ...
    safety_case_study_experiment = CaseStudySafetyExperiment(
        ...
        n_sims_per_start=1,
        t_sim=t_sim,
        plot_x_indices=[AdaptivePusherSliderStickingForceInput.S_X, AdaptivePusherSliderStickingForceInput.S_Y, AdaptivePusherSliderStickingForceInput.S_THETA],
        plot_x_labels=["$s_x$", "$s_y$", "$s_\\theta$"],
    )
...
def main(args):
    ...
    # Constants
    t_sim = 15.0
    #controllers_to_test = ["aclbf", "nominal", "mpc", "trajopt2"]
    controllers_to_test = ["aclbf"]
    ...
    # Create trajectories
    _, theta0, theta_hat0, scen0 = create_initial_states_parameters_and_estimates(
        dynamics_model, x0,
        n_sims_per_start=1,
    )
    ...
    if "aclbf" in controllers_to_test:
    ...
    if "nominal" in controllers_to_test:
    ...
    if "trajopt2" in controllers_to_test:
        ...
        obs_rad = dynamics_model.nominal_scenario["obstacle_radius"]
        constraints = []
        constraints.append(
            (
        ...
        trajopt2_results_df, trajopt2_synthesis_times, X_trajopt, U_trajopt, t_trajopt, trajopt_durations = safety_trajopt_exp.run(
            ...
            lsm_update,
            Tf=t_sim,
            P=np.diag([1.0e3, 1.0e3, 1.0e3]),
            Q=np.diag([1.0e2, 1.0e2, 1.0e1]),
            R=np.diag([0.0e-2, 0.0e-2]),
            N_timepts=20,
            u0=np.array([-1.0, 1.0]),
    ...
    with open("../datafiles/pusher_slider_sticking/safety_case_study_results.txt", "w") as f:
    ...
    save_timing_data_table(
        "../datafiles/pusher_slider_sticking/safety_case_study_timing_results.txt",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--version_number', type=str, default="0",
