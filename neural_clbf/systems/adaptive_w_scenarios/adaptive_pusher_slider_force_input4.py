...
from neural_clbf.systems.adaptive_w_scenarios import (
    ControlAffineParameterAffineSystem3,
)
...
class AdaptivePusherSliderStickingForceInput4(ControlAffineParameterAffineSystem3):
    """
    ...
    def __init__(
        ...
        # Then initialize
        super().__init__(nominal_scenario, V_Theta, dt, controller_dt, device=device)
        ...
        if V_scenario is None:
            ...
            for i in range(self.n_obstacles):
                box_limits += [
                    (x_lb[AdaptivePusherSliderStickingForceInput4.S_X], x_ub[AdaptivePusherSliderStickingForceInput4.S_X]),
                    (x_lb[AdaptivePusherSliderStickingForceInput4.S_Y], x_ub[AdaptivePusherSliderStickingForceInput4.S_Y]),
                    # (0.0, 0.1),
                ]
            ...
            P_scenario = pc.box2poly(box_limits)

            self.V_scenario = torch.tensor(pc.extreme(P_scenario), device=self.device)
        else:
            self.V_scenario = V_scenario

        assert self.V_scenario.shape[1] == self.n_scenario, f"V_scenario has dimension {V_scenario.dim}; expected dimension {self.n_scenario}"
    ...
    @property
    def n_dims(self) -> int:
        return AdaptivePusherSliderStickingForceInput4.N_DIMS

    @property
    def angle_dims(self) -> List[int]:
        return [AdaptivePusherSliderStickingForceInput4.S_THETA]
    ...
    def n_controls(self) -> int:
        return AdaptivePusherSliderStickingForceInput4.N_CONTROLS

    @property
    def n_params(self) -> int:
        return AdaptivePusherSliderStickingForceInput4.N_PARAMETERS

    @property
    def n_scenario(self) -> int:
        return self.n_obstacles * 2
    ...
    def state_limits(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        upper_limit[AdaptivePusherSliderStickingForceInput4.S_X] = 1.0
        upper_limit[AdaptivePusherSliderStickingForceInput4.S_Y] = 1.0
        upper_limit[AdaptivePusherSliderStickingForceInput4.S_THETA] = torch.pi
    ...
    def U_vertices(self) -> torch.Tensor:
        ...
        V_U = torch.tensor([
            [0.0, 0.0],
            [self.max_force, self.ps_cof* self.max_force],
            [self.max_force, -self.ps_cof* self.max_force],
        ]).to(self.device)
    ...
    @property
    def scenario_set_vertices(self) -> pc.Polytope:
        """
    ...
    def unsafe_mask(self, x: torch.Tensor, theta: torch.Tensor, s_vec: torch.Tensor) -> torch.Tensor:
        """
        Return the mask of x indicating safe regions for this system
    ...
    def goal_mask(self, x: torch.Tensor, theta: torch.Tensor, s_vec: torch.Tensor) -> torch.Tensor:
        """
        Return the mask of x indicating goal regions for this system

        args:
            x: a tensor of (batch_size, self.n_dims) points in the state space
            theta: a tensor of (batch_size, self.n_dims) parameters defining the unknown parameters of the system.
        returns:
            a tensor of (batch_size,) booleans indicating whether the corresponding
            point is in this region.
        """
        ...
        goal_pose = self.goal_point(theta, s_vec)
    ...
    def goal_point(self, theta: torch.Tensor, s_vec: torch.Tensor) -> torch.Tensor:
        """
        goal_point
        Description:
        ...
        goal[:, AdaptivePusherSliderStickingForceInput4.S_X] = 0.5
        goal[:, AdaptivePusherSliderStickingForceInput4.S_Y] = 0.5
        goal[:, AdaptivePusherSliderStickingForceInput4.S_THETA] = 0.0
    ...
    def sample_unsafe(
        ...
        obst_sample_indices = torch.randint(
            low=0, high=self.n_obstacles, size=(batch_size,),
        ).type_as(s_unsafe)

        # Sample Abstract parameters of each obstacle
        ...
        x_abst_unsafe_np = self.get_N_samples_from_polytope(V_unsafe, num_samples)
        if torch.get_default_dtype() == torch.float32:
            x_abst_unsafe_np = np.float32(x_abst_unsafe_np)
        x_abst_unsafe = torch.tensor(x_abst_unsafe_np, device=self.device)
        ...
        for obst_index in range(self.n_obstacles):
            ...
            x_unsafe[obst_i_indices, AdaptivePusherSliderStickingForceInput4.S_X] = \
                torch.cos(x_abst_unsafe[obst_i_indices, ABSTRACT_THETA]) * r_i + \
                s_unsafe[obst_i_indices, obst_center_x_index]

            x_unsafe[obst_i_indices, AdaptivePusherSliderStickingForceInput4.S_Y] = \
                torch.sin(x_abst_unsafe[obst_i_indices, ABSTRACT_THETA]) * r_i + \
                s_unsafe[obst_i_indices, obst_center_y_index]

            x_unsafe[obst_i_indices, AdaptivePusherSliderStickingForceInput4.S_THETA] = \
    ...
    def sample_goal(
        ...
        # Sample concrete states from goal
        x_goal = torch.zeros((batch_size, self.n_dims), device=self.device).type_as(s_goal)

        # Save indices for vectorized operations
        goal_center = self.goal_point(
            torch.zeros((batch_size, self.n_params), device=self.device).type_as(s_goal),
            s_goal,
        )

        # Create points in each circular obstacle
        r_i = x_abst_goal[:, ABSTRACT_RADIUS] * self.goal_radius
        x_goal[:, AdaptivePusherSliderStickingForceInput4.S_X] = \
            torch.cos(x_abst_goal[:, ABSTRACT_THETA]) * r_i + \
            goal_center[:, AdaptivePusherSliderStickingForceInput4.S_X]

        x_goal[:, AdaptivePusherSliderStickingForceInput4.S_Y] = \
            torch.sin(x_abst_goal[:, ABSTRACT_THETA]) * r_i + \
            goal_center[:, AdaptivePusherSliderStickingForceInput4.S_Y]

        x_goal[:, AdaptivePusherSliderStickingForceInput4.S_THETA] = \
    ...
    def _g(self, x: torch.Tensor, s_vec: torch.Tensor) -> torch.Tensor:
        ...
        # States
        s_x = x[:, AdaptivePusherSliderStickingForceInput4.S_X]
        s_y = x[:, AdaptivePusherSliderStickingForceInput4.S_Y]
        s_theta = x[:, AdaptivePusherSliderStickingForceInput4.S_THETA]

        # Algorithm
        g[:, AdaptivePusherSliderStickingForceInput4.S_X, AdaptivePusherSliderStickingForceInput4.F_X] = torch.cos(s_theta) * a
        g[:, AdaptivePusherSliderStickingForceInput4.S_X, AdaptivePusherSliderStickingForceInput4.F_Y] = -torch.sin(s_theta) * a

        g[:, AdaptivePusherSliderStickingForceInput4.S_Y, AdaptivePusherSliderStickingForceInput4.F_X] = torch.sin(s_theta) * a
        g[:, AdaptivePusherSliderStickingForceInput4.S_Y, AdaptivePusherSliderStickingForceInput4.F_Y] = torch.cos(s_theta) * a
    ...
    def _G(self, x: torch.Tensor, s_vec: torch.Tensor) -> torch.Tensor:
        ...
        G[:, AdaptivePusherSliderStickingForceInput4.S_THETA, AdaptivePusherSliderStickingForceInput4.F_X, AdaptivePusherSliderStickingForceInput4.C_Y] = -b
        G[:, AdaptivePusherSliderStickingForceInput4.S_THETA, AdaptivePusherSliderStickingForceInput4.F_Y, AdaptivePusherSliderStickingForceInput4.C_X] = b
    ...
    def u_nominal(
        ...
        s_x = x[:, AdaptivePusherSliderStickingForceInput4.S_X]
        s_y = x[:, AdaptivePusherSliderStickingForceInput4.S_Y]
        s_th = x[:, AdaptivePusherSliderStickingForceInput4.S_THETA]
        ...
        u_nominal = torch.zeros((batch_size, AdaptivePusherSliderStickingForceInput4.N_CONTROLS)).to(self.device)
        ...
        u_nominal[des_angle_in_fc_mask, AdaptivePusherSliderStickingForceInput4.F_X] = \
            torch.cos(des_angle[des_angle_in_fc_mask])
        u_nominal[des_angle_in_fc_mask, AdaptivePusherSliderStickingForceInput4.F_Y] = \
    ...
    def estimate_goal(
        ...
        estimate_goal
        Description:
            This function estimates the goal pose from the current state,
            parameters and the nominal control input which is dependent on
            the true theta.
        """
        # Constants
        bs = x.shape[0]
        ...
        # Formulate LP for determining the true desired directions
        desired_pos = torch.zeros((bs, 2)).type_as(x)
        for batch_idx in range(bs):
            # Create variables
            model = gp.Model("goal_estimation")
            x_g = model.addMVar(
                (2,),
            ...
            # Create constraints
            model.addConstr(
                Theta.A[:, 2:] @ x_g <= Theta.b - Theta.A[:, :2] @ theta_hat[batch_idx, :2].numpy(),
            )

            # Create objective
            model.setObjective(
                (x_g - curr_pos) @ u_nom[batch_idx, :].numpy(),
                GRB.MAXIMIZE,
            )
            ...
            if model.status != GRB.OPTIMAL:
                raise RuntimeError("Goal estimation LP was not solved to optimality.")

            # Extract results
            x_g_result = torch.tensor(x_g.x)

            desired_pos[batch_idx, :] = x_g_result
