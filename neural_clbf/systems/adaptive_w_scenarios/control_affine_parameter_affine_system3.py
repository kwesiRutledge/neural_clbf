...
class ControlAffineParameterAffineSystem3(ABC):
    """
    ...
    def __init__(
        ...
        if theta is None:
            theta = self.get_N_samples_from_polytope(self.V_Theta, 1) # Return single vector representing unknown parameters.
        self.theta = theta
    ...
    def U_vertices(self) -> torch.Tensor:
        ...
        Description:
            This abstract property returns the vertices defining the polytope U, which is the set of all
            allowable control inputs for the system.
        Returns:
            V_U: A n_V x n_controls tensor defining the n_V vertices that define the polytope U.
        """
        pass

    @abstractproperty
    def scenario_set_vertices(self) -> torch.Tensor:
        """
        Description:
            Returns a number of vertices whose convex hull represents the set of allowable scenarios
        Returns:
            V_U: A n_V x n_scenario tensor defining the n_V vertices that define the polytope scenario_set.
        """
        pass

    @property
    def scenario_limits(self) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Description:
    ...
    def sample_Theta_space(self, num_samples: int) -> torch.Tensor:
        ...
        theta_samples_np = self.get_N_samples_from_polytope(self.V_Theta, num_samples)
    ...
    def sample_scenario_space(self, num_samples: int) -> torch.Tensor:
        ...
        scenario_samples_np = self.get_N_samples_from_polytope(self.scenario_set_vertices, num_samples)
        if torch.get_default_dtype() == torch.float32:
    ...
    def get_N_samples_from_polytope(self, V_P: torch.Tensor, N_samples):
        """
        get_N_samples_from_polytope
        Description:
            This function retrieves N samples from the polytope specified by the convex hull of points in V_P.
            Used to more efficiently produce samples (only have to compute extremes once.)
        Args:
            V_P: A torch tensor of size n_V x P.Dim containing the vertices whose convex hull forms the polytope P.
