"""
adaptive_pusher_slider_sticking_force_input_more_obstacles.py
Description:
...
class AdaptivePusherSliderStickingForceInput_MoreObstacles(ControlAffineParameterAffineSystem):
    """
    ...
    def validate_scenario(self, s: Scenario) -> bool:
        ...
        valid = valid and ("obstacle_centers" in s)
        valid = valid and ("obstacle_radii" in s)

        valid = valid and (len(s["obstacle_centers"]) == len(s["obstacle_radii"])) # Same number of obstacle centers as radii

        return valid

    @property
    def n_dims(self) -> int:
        return AdaptivePusherSliderStickingForceInput_MoreObstacles.N_DIMS

    @property
    def angle_dims(self) -> List[int]:
        return [AdaptivePusherSliderStickingForceInput_MoreObstacles.S_THETA]
    ...
    def n_controls(self) -> int:
        return AdaptivePusherSliderStickingForceInput_MoreObstacles.N_CONTROLS

    @property
    def n_params(self) -> int:
        return AdaptivePusherSliderStickingForceInput_MoreObstacles.N_PARAMETERS
    ...
    def state_limits(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        upper_limit[AdaptivePusherSliderStickingForceInput_MoreObstacles.S_X] = 1.0
        upper_limit[AdaptivePusherSliderStickingForceInput_MoreObstacles.S_Y] = 1.0
        upper_limit[AdaptivePusherSliderStickingForceInput_MoreObstacles.S_THETA] = torch.pi
    ...
    def safe_mask(self, x: torch.Tensor, theta: torch.Tensor) -> torch.Tensor:
        ...
        n_obstacles = len(self.nominal_scenario["obstacle_centers"])
        ...
        for i in range(n_obstacles):
            obst_center_i = torch.tensor(
                self.nominal_scenario["obstacle_centers"][i]
            ).to(self.device)
            obst_radius_i = self.nominal_scenario["obstacle_radii"][i]


            displacement_to_obst_center = x[:, :2] - obst_center_i.repeat(batch_size, 1)

            # distance to obstacle center is greater than radius + half of the length of the square
            dist_to_obst_center_big_enough = displacement_to_obst_center.norm(dim=-1) >= (obst_radius_i + self.s_length/2)
    ...
    def unsafe_mask(self, x: torch.Tensor, theta: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(n_obstacles):
            ...
            # distance to obstacle center is greater than radius + half of the length of the square
            dist_to_obst_center_too_small = displacement_to_obst_center.norm(dim=-1) >= (
                        obst_radius_i + self.s_length / 2)
    ...
    def goal_point(self, theta: torch.Tensor) -> torch.Tensor:
        ...
        goal[:, AdaptivePusherSliderStickingForceInput_MoreObstacles.S_THETA] = 0.0
    ...
    def _g(self, x: torch.Tensor, params: Scenario) -> torch.Tensor:
        ...
        # States
        s_x = x[:, AdaptivePusherSliderStickingForceInput_MoreObstacles.S_X]
        s_y = x[:, AdaptivePusherSliderStickingForceInput_MoreObstacles.S_Y]
        s_theta = x[:, AdaptivePusherSliderStickingForceInput_MoreObstacles.S_THETA]

        # Algorithm
        g[:, AdaptivePusherSliderStickingForceInput_MoreObstacles.S_X, AdaptivePusherSliderStickingForceInput_MoreObstacles.F_X] = torch.cos(s_theta) * a
        g[:, AdaptivePusherSliderStickingForceInput_MoreObstacles.S_X, AdaptivePusherSliderStickingForceInput_MoreObstacles.F_Y] = -torch.sin(s_theta) * a

        g[:, AdaptivePusherSliderStickingForceInput_MoreObstacles.S_Y, AdaptivePusherSliderStickingForceInput_MoreObstacles.F_X] = torch.sin(s_theta) * a
        g[:, AdaptivePusherSliderStickingForceInput_MoreObstacles.S_Y, AdaptivePusherSliderStickingForceInput_MoreObstacles.F_Y] = torch.cos(s_theta) * a
    ...
    def _G(self, x: torch.Tensor, params: Scenario) -> torch.Tensor:
        ...
        G[:, AdaptivePusherSliderStickingForceInput_MoreObstacles.S_THETA, AdaptivePusherSliderStickingForceInput_MoreObstacles.F_X, AdaptivePusherSliderStickingForceInput_MoreObstacles.C_Y] = -b
        G[:, AdaptivePusherSliderStickingForceInput_MoreObstacles.S_THETA, AdaptivePusherSliderStickingForceInput_MoreObstacles.F_Y, AdaptivePusherSliderStickingForceInput_MoreObstacles.C_X] = b
    ...
    def u_nominal(
        ...
        s_x = x[:, AdaptivePusherSliderStickingForceInput_MoreObstacles.S_X]
        s_y = x[:, AdaptivePusherSliderStickingForceInput_MoreObstacles.S_Y]
        s_th = x[:, AdaptivePusherSliderStickingForceInput_MoreObstacles.S_THETA]
        ...
        u_nominal = torch.zeros((batch_size, AdaptivePusherSliderStickingForceInput_MoreObstacles.N_CONTROLS)).to(self.device)
        ...
        u_nominal[des_angle_in_fc_mask, AdaptivePusherSliderStickingForceInput_MoreObstacles.F_X] = \
            torch.cos(des_angle[des_angle_in_fc_mask])
        u_nominal[des_angle_in_fc_mask, AdaptivePusherSliderStickingForceInput_MoreObstacles.F_Y] = \
    ...
    def plot_single(self,
        ...
        if show_obstacle:
            ...
            plot_objects["obstacles"] = []
            for obstacle_index in range(len(s["obstacle_centers"])):
                obstacle_x, obstacle_y = s["obstacle_centers"][obstacle_index]
                obstacle_radius = s["obstacle_radii"][obstacle_index]
    ...
    def plot(self, x: torch.Tensor, theta: torch.Tensor,
        ...
        # Find maximum and minimum values for position of obstacles
        max_obstacle_x = torch.max(torch.tensor([point[0] for point in self.nominal_scenario["obstacle_centers"]]))
        min_obstacle_x = torch.max(torch.tensor([point[0] for point in self.nominal_scenario["obstacle_centers"]]))

        max_obstacle_y = torch.max(torch.tensor([point[1] for point in self.nominal_scenario["obstacle_centers"]]))
        min_obstacle_y = torch.max(torch.tensor([point[1] for point in self.nominal_scenario["obstacle_centers"]]))

        max_obstacle_radius = torch.max(torch.tensor(
            self.nominal_scenario["obstacle_radii"],
        ))
        ...
        if limits is None:
            ...
            if show_obstacle:
                # Incorporate obstacle in limits
                limits[0][0] = min(limits[0][0], min_obstacle_x - 2*max_obstacle_radius)
                limits[0][1] = max(limits[0][1], max_obstacle_x + 2*max_obstacle_radius)
                limits[1][0] = min(limits[1][0], min_obstacle_y - 2*max_obstacle_radius)
                limits[1][1] = max(limits[1][1], max_obstacle_y + 2*max_obstacle_radius)
