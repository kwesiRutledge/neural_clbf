...
class RolloutParameterConvergenceExperiment(Experiment):
    ...
    def __init__(
        ...
        t_sim: float = 5.0,
        hide_state_rollouts: bool = False,
    ):
        ...
        """
        super(RolloutParameterConvergenceExperiment, self).__init__(name)
        ...
        self.t_sim = t_sim
        self.hide_state_rollouts = hide_state_rollouts
        ...
        for tstep in prog_bar_range:
            ...
            for sim_index in range(n_sims):
                ...
                if V is not None:
                    ...
                    log_packet["V_oracle"] = V_oracle[sim_index].cpu().numpy().item()
    ...
    def plot(
        ...
        if self.hide_state_rollouts:
            num_plots -= 1

        # Create the figure which will hold all trajectories
        fig = plt.figure()

        if self.hide_state_rollouts:
            error_ax = fig.add_subplot(100 + 10 * num_plots + 1)
        else:
            rollout_ax = fig.add_subplot(101 + 10 * num_plots, projection='3d')
            error_ax = fig.add_subplot(100 + 10 * num_plots + num_plots)
        ...
        if "h" in results_df:
            h_ax = fig.add_subplot(103 + 10*(num_plots-1))
            h_violation_ax = fig.add_subplot(104 + 10*num_plots)

        if "V" in results_df:
            V_ax = fig.add_subplot(100 + 10*num_plots + num_plots) #ax[num_plots - 1]

        # Plot the rollouts
        if not self.hide_state_rollouts:
            self.plot_state_rollouts(rollout_ax, results_df, controller_under_test.dynamics_model) # First of the state

        self.plot_error(results_df, error_ax) # Then for the parameter estimation error

        if "h" in results_df:
            self.plot_barrier(results_df, h_ax, h_violation_ax) # Plot the barrier function if applicable

        # Plot the lyapunov function if applicable
        if "V" in results_df:
            self.plot_aclf_rollouts(results_df, V_ax)
    ...
    def plot_state_rollouts(self, results_df: pd.DataFrame, state_rollout_ax: plt.axis, dynamics_model):
        """
        self.plot_state_rollouts(results_df, state_rollout_ax)

        Description:
            Plots the state rollouts (i.e., the trajectory of the axes self.plot_x_labels[0], self.plot_x_labels[1] and
            self.plot_x_labels[2]) for each simulation in the results_df dataframe.
        Returns:
            None
        """
        ...
        for plot_idx, sim_index in enumerate(results_df["Simulation"].unique()):
            ...
            state_rollout_ax.set_xlabel(self.plot_x_labels[0])
            state_rollout_ax.set_ylabel(self.plot_x_labels[1])
            state_rollout_ax.set_zlabel(self.plot_x_labels[2])

            # Plot initial conditions
            state_rollout_ax.scatter(
        ...
        state_rollout_ax.legend([], [], frameon=False)

        # Plot the environment
        dynamics_model.plot_environment(state_rollout_ax)

    def plot_error(self, results_df: pd.DataFrame, error_ax: plt.axis):
        """
        self.plot_error(results_df, error_ax)
        Description:
            Plots the estimation error of the parameters for each simulation in the results_df dataframe.
        Returns:
            None
        """
        # Constants
        ...
        for plot_idx, sim_index in enumerate(results_df["Simulation"].unique()):
            ...
            error_ax.set_ylabel("$\| \\theta(t) - \\hat{\\theta}(t) \|$")

    def plot_barrier(self, results_df: pd.DataFrame, barrier_ax: plt.axis, barrier_violation_ax: plt.axis = None):
        """
        self.plot_barrier(results_df, barrier_ax)
        Description:
            Plots the barrier function for each simulation in the results_df dataframe.
        Returns:
            None
        """
        # Constants
        ...
        for plot_idx, sim_index in enumerate(results_df["Simulation"].unique()):
            ...
            barrier_ax.set_ylabel("$h$")
            barrier_ax.set_xlabel("t")
            # Remove the legend -- too much clutter
            barrier_ax.legend([], [], frameon=False)

            # Plot a reference line at h = 0
            barrier_ax.plot([0, results_df["t"].max()], [0, 0], color="k")
            ...
            barrier_violation_ax.plot(
                results_df[sim_mask]["t"][:-1].to_numpy(),
                h_violation,
                linestyle=":",
                color=sns.color_palette()[plot_idx],
            )
            barrier_violation_ax.set_ylabel("$h$ violation")


    def plot_aclf_rollouts(self, results_df: pd.DataFrame, V_rollout_ax: plt.axis):
        """
        self.plot_aclf_rollouts(results_df, V_rollout_ax)
        Description:
            Plots the ACLF rollouts for each simulation in the results_df dataframe.
        Returns:
            None
        """
        # Constants
        ...
        V_rollout_ax.set_ylabel("$V$")
        V_rollout_ax.set_xlabel("t")

        # Plot a reference line at V = 0
        V_rollout_ax.plot([0, results_df.t.max()], [0, 0], color="k")

        # Legend!
        V_rollout_ax.legend([], [], frameon=False) # Remove legend; too much clutter

        # Plotting the oracle CLF
        for plot_idx, sim_index in enumerate(results_df["Simulation"].unique()):
            sim_mask = results_df["Simulation"] == sim_index

            V_rollout_ax.plot(
                results_df[sim_mask]["t"].to_numpy(),
                results_df[sim_mask]["V_oracle"].to_numpy(),
                linestyle="dashed",
                # marker="+",
                markersize=5,
                color=sns.color_palette()[plot_idx],
