"""Obstacle Avoidance Experiment for Pusher-Slider Models"""
...
class PusherObstacleAvoidanceExperiment(Experiment):
    """An experiment for plotting obstacle avoidance performance of pusher-slider controllers"""
    ...
    def __init__(
        ...
        """
        super(PusherObstacleAvoidanceExperiment, self).__init__(name)
    ...
    def run(self, controller_under_test: "Controller") -> pd.DataFrame:
        ...
        assert (
            isinstance(controller_under_test.dynamics_model, StickingPusherSlider)
        ...
        prog_bar_range = tqdm.trange(0, T, desc="Avoid-Obstacle-Curve", leave=True)
        for tstep in prog_bar_range:
            # Update the reference path to trace an S curve
            # omega_ref = 1.5 * np.sin(tstep * delta_t)
            # psi_ref += delta_t * omega_ref
            s_x_ref = params["s_x_ref"]
            s_y_ref = params["s_y_ref"]
            ...
            x = x_current[0, controller_under_test.dynamics_model.S_X]
            y = x_current[0, controller_under_test.dynamics_model.S_Y]
            x_err = x - s_x_ref
            y_err = y - s_y_ref
    ...
    def plot(
        ...
        fig_handle = ("Reachability to Goal", fig)
