...
class RolloutStateParameterSpaceExperimentMultiple(Experiment):
    ...
    def run(self, controller_under_test: "Controller") -> pd.DataFrame:
        ...
        # Choose scenario vectors
        if self.scen is None:
            scen = controller_under_test.dynamics_model.sample_scenario_space(n_sims)
        else:
            scen = self.scen
        ...
        for tstep in prog_bar_range:
            ...
            if tstep % controller_update_freq == 0:
                ...
                u_current, r_current = controller_under_test.solve_CLF_QP(x_current, theta_hat_current, scen)
                end_time = time.time()
                controller_calls += 1
            ...
            if hasattr(controller_under_test, "V") and h is None:
                V = controller_under_test.V(x_current, theta_hat_current, scen)  # type: ignore
