...
def tabulate_number_of_reaches(
    ...
    for plot_idx, sim_index in enumerate(results_df["Simulation"].unique()):
        ...
        sim_results_scen = np.vstack(
            np.array(results_df[sim_mask]["scen"])
        )
        sim_results_scen = torch.tensor(sim_results_scen)
...
def plot_rollouts(
    results_df: pd.DataFrame,
    rollout_ax: plt.Axes,
    dynamical_system: ControlAffineParameterAffineSystem2,
    ...
    for plot_idx, sim_index in enumerate(results_df["Simulation"].unique()):
        ...
        scen = np.stack(results_df[sim_mask]["scen"].to_numpy())
        # print(sim_index)
        # print("theta.shape = ", theta.shape)
        goal_point = dynamical_system.goal_point(
            torch.tensor(theta[0, :]).reshape((1, dynamical_system.n_params)),
            torch.tensor(scen[0, :]).reshape((1, dynamical_system.n_scenario)),
        )
...
def plot_error_to_goal(
    ...
    for plot_idx, sim_index in enumerate(results_df["Simulation"].unique()):
        ...
        scen_sim_i = torch.tensor(np.vstack(results_df[sim_mask]["scen"]).T)

        goal_sim_i = dynamical_system.goal_point(theta_sim_i.T, scen_sim_i.T).T
...
def create_initial_states_parameters_and_estimates(
    ...
    scen_sim_start = dynamics.sample_scenario_space(n_sims)

    return x_sim_start, theta_sim_start, theta_hat_sim_start, scen_sim_start
